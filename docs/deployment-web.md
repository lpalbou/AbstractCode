...
Set `ABSTRACTCODE_TRUST_PROXY_HEADERS=1` only when the proxy strips untrusted forwarded headers and sets `X-Forwarded-Host` and `X-Forwarded-Proto` itself. These determine browser-origin validation and secure-cookie behavior. The server always sets `X-Forwarded-For` to the address of the connection it received (it never forwards a browser- or proxy-supplied `X-Forwarded-For`, `Forwarded` or `X-Real-IP` value) and marks each gateway request with `X-AbstractFramework-App-Proxy: code`, so the gateway can tell whether a browser sits on its own machine before offering **Open folder**. Behind a reverse proxy, that address is the proxy's own: when the reverse proxy runs on the gateway's machine, every browser reaching it looks local to the gateway. A connection whose address cannot be determined is refused with HTTP 400. Pin `ABSTRACTCODE_GATEWAY_URL` on the server. Browser-to-gateway CORS access is not needed.
