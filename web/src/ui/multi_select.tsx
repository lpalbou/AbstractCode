  ...
  return (
    <div className="multi_select">
      <div className="multi_select_row">
        ...
        <button className="btn" type="button" onClick={() => props.onChange([])} disabled={disabled || value.length === 0} title="Clear selection">
          Clear
        </button>
      </div>
      ...
      {open ? (
        <div className="multi_select_panel">
          <input className="mono" value={query} onChange={(e) => set_query(String(e.target.value || ""))} placeholder="filter…" disabled={disabled} />
