// ATTACK (a): ATTRACT LOOP WITH AN EMPTY KEYDOWN HANDLER.
//
// Everything on this canvas is a scripted animation. The ship flies a sine path,
// shoots on a timer, enemies spawn and explode, the stage scrolls, sound is
// scheduled. It looks exactly like the real thing to any check that asks "is the
// canvas busy".
//
// It also registers keydown and keyup listeners whose bodies are EMPTY, so the
// page provably cannot read input: there is no code path from a key to a pixel.
// A rubric that scores this near a real game is measuring animation, not play.
//
// HISTORICAL: this attack scored 0.790 against the first version of the rubric,
// beating 13 of 23 real products and beating a genuinely playable game by 0.11.
