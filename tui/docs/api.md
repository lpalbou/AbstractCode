...
| `--permissions <level>` | Tool permissions for this launch: `read` \| `write` \| `all`. Applies to interactive launches and `exec` alike, for that launch only — never saved (`/permissions` is what saves a level). Interactive: a startup notice says the level in force ("permissions: write (from --permissions, this launch)") | the saved level of the session |
| `--require-approval <t>` | Gate these tools regardless of level (comma list, repeatable), for this launch only (not saved). Interactive: the app asks before each one, and a startup notice names them; `exec` denies them | — |
