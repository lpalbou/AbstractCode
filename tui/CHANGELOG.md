...
- **`--permissions` and `--require-approval` now apply to the interactive
  ...
  changed), and a notice says what is in force.
- **The "gateway runs on this machine" notice names the folder actually
  sent.** With `--no-workspace` it says no folder is sent; when the launch
  folder does not exist it says so instead of implying a working folder.
